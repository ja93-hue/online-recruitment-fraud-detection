...
SUBTLE_SCAMS = [
    {
        ...
        Salary: INR 35,000 - INR 50,000 per month
        ...
        (INR 4,500), which includes software access and training materials. This cost will be
...
ALL_CASES = [
    pytest.param(category, case, id=f"{category}: {case['name']}")
    for category, cases in ALL_TESTS
    for case in cases
]
...
@pytest.mark.parametrize(("category", "job"), ALL_CASES)
def test_detection_case(detector, category, job):
    result = detector.predict(job["text"])
    actual = "fraud" if result["prediction"] == 1 else "legitimate"

    assert actual == job["expected"], (
        f"{category} - {job['name']}: expected {job['expected']}, "
        f"got {actual} ({result['confidence']:.1f}%)"
    )
